...
print ("START")
# load data from file	
with open(rootPath+'/config.json', mode='r') as f:
...
class TooltipClass(QtWidgets.QDialog):
	def __init__(self, parent, tooltip_widget):
		super(TooltipClass, self).__init__(parent)
		...
		self.lay.addWidget(self.tooltip_widget)
	...
	def updateWidget(self):
		# set text
		textFile = rootPath+'/ui/tooltips/%s.txt' %self.objectName
		...
		name = f.readline()
		descr = f.read()
		self.tooltip_widget.label.setText(name)
		self.tooltip_widget.label_2.setText(descr)		
		
		# set image
		imgPath = rootPath+'/ui/tooltips/%s.png' %self.objectName
		if os.path.isfile(imgPath) :
			actualImage = QtGui.QImage(imgPath)
			pixmap = QtGui.QPixmap(imgPath)
			#pixmap = pixmap.scaled(500, 500, QtCore.Qt.KeepAspectRatio) 
			self.tooltip_widget.image_label.setPixmap(pixmap)
			self.tooltip_widget.image_label.setVisible(1)
		else:
			self.tooltip_widget.image_label.setVisible(0)
...
class EventClass(QtWidgets.QWidget):
	def __init__(self, layout, win=None, toolTip_widget=None):
		super(EventClass, self).__init__()

		self.win = win
		self.helpMode = False
		self.curTooltip = TooltipClass(self, toolTip_widget)
		self.cur_widget = None
		
		layout.insertWidget(0, self)
		
		#p = rootPath+'/ui/tooltips/'
		#for f in os.listdir(p):
			#if f.split(".")[-1] == "txt":
				#n = f.split(".")[0]
				#eval("self.win.%s.installEventFilter(self)" %n)
		self.win.installEventFilter(self)
		#self.win.modules_treeWidget.installEventFilter(self)

	def eventFilter(self, widget, event):
		pos = QtGui.QCursor.pos()
		#cur_widget = QtWidgets.qApp.widgetAt(pos)
		#print 111, event.type()

		try: # try needed for removing sometime warnings
			if self.helpMode:
				self.curTooltip.move(pos.x()+15, pos.y())
				widget_underCursor = QtWidgets.qApp.widgetAt(pos)
				if not widget_underCursor or widget_underCursor.objectName() == "":
					return
				if self.cur_widget != widget_underCursor:
					self.cur_widget = widget_underCursor
					#print self.cur_widget.objectName()
					self.curTooltip.objectName = self.cur_widget.objectName()
					self.curTooltip.updateWidget()
	
			else:
				self.curTooltip.hide()
		except: pass
		
class RigBrushEventClass(QtCore.QObject):
	def __init__(self, main=None):
		super(RigBrushEventClass, self).__init__()
	...
	def eventFilter(self, widget, event):
		...
		e = event.type()
		#print 333, e
		#print self.main.moveMode, cmds.draggerContext('pickShapeCtx', exists=True)
		#if e == QtCore.QEvent.Type.GraphicsSceneMousePress:
			#print "PRESS"
		if event.type() == QtCore.QEvent.KeyPress:
			key = event.key()
			print ("PRESS", key, key == QtCore.Qt.Key_Escape)
			
			#if self.main.moveMode:
				#QtWidgets.QApplication.setOverrideCursor(QtGui.QCursor(QtCore.Qt.CursorShape(3)))
			
			if key == QtCore.Qt.Key_Escape:
				self.main.moveMode = False
...
class ActionClass(QtWidgets.QAction):
	def __init__(self, win=None):
		super(ActionClass, self).__init__(win)	
...
class ButtonAction(QtWidgets.QWidgetAction):
	def __init__(self,	parent, text):
		...
		b.setFlat(True)
		b.setIcon(QtGui.QIcon(rootPath+'/ui/icons/delete_icon.png'))
...
class EventFilterClass(QtCore.QObject):
	...
	def eventFilter(self, obj, event):
		...
		if e == QtCore.QEvent.Type.GraphicsSceneMousePress:
			...
			itemUnderMouse =  self.scene.itemAt(scenePos, QtGui.QTransform())
			#print "press"
			if itemUnderMouse:
				#print cmds.getModifiers() == 4 , event.button() == QtCore.Qt.MouseButton.RightButton
				if cmds.getModifiers() == 4 and event.button() == QtCore.Qt.MouseButton.LeftButton:
					if self.connection:
						self.target = cmds.nodeEditor(self.ned, q=1, feedbackNode=1)
						self.showMenu(event)
						#self.clickPos = event.pos()
				#print 0, self.mapToGlobal(self.clickPos)	
				#p = event.pos() # relative to widget
				#self.gp = self.view.mapToGlobal(p) # relative to screen
				#self.rw = self.view.mapFromGlobal(gp) # relative to window	

				currentPos = event.screenPos()
				scenePos = event.scenePos()
				self.clickPos = scenePos
				#print 11, currentPos, scenePos


			else:
				#evt = QtGui.QKeyEvent(QtCore.QEvent.KeyPress, QtCore.Qt.Key_Escape, QtCore.Qt.NoModifier)
				#QtCore.QCoreApplication.sendEvent(self, evt)	
				#print self.clickPos
				pass

		# disconnect on click on empty area
		elif e == QtCore.QEvent.Type.GraphicsSceneMouseRelease:
			#print "release"
			scenePos = event.scenePos()
			QtTest.QTest.keyPress(self.view, QtCore.Qt.Key_Escape)
			QtTest.QTest.keyRelease(self.view, QtCore.Qt.Key_Escape)	
		elif e == QtCore.QEvent.Type.GraphicsSceneMouseMove:
			#if cmds.getModifiers() == 4 and event.button() == QtCore.Qt.MouseButton.LeftButton:
			QtTest.QTest.keyPress(self.view, QtCore.Qt.Key_Escape)
			QtTest.QTest.keyRelease(self.view, QtCore.Qt.Key_Escape)
			#print "move"
...
class Main():
	...
	def __init__(self):
		...
		self.uiFilePath = rootPath+'//ui//mainWindow.ui'
		self.aboutUiFilePath = rootPath+'//ui//aboutWindow.ui'
		self.setsUiFilePath = rootPath+'//ui//setsWindow.ui'
		self.humanIkUiFilePath = rootPath+'//ui//humanIkWindow.ui'
		self.moduleBuilderUiFilePath = rootPath+'//ui//moduleBuilderWindow.ui'
		self.matchrigUiFilePath = rootPath+'//matchrig//matchrigWindow.ui'
		self.attributesUiFilePath = rootPath+'//ui//attributesWindow.ui'
		self.driverUiFilePath = rootPath+'//ui//driver.ui'
		
		app = QtWidgets.QApplication.instance()
		self.win = self.loadUiWidget(self.uiFilePath)		
		app.exec_()
		
		self.help_widget = self.loadUiWidget(rootPath+'//ui//help_widget.ui')		
		...
		for m in moduleNames:
			#try:
			path = rootPath+'//modules//'+m+'//'+m+'_widget.ui'
			#print 111, path, os.path.isfile(path)
			if os.path.isfile(path):
				w = self.loadUiWidget(path)
		...
		#self.tools = tools.makeTestAnimation()
		#self.screen_resolution = app.desktop().screenGeometry()
		self.curParents = parents.Parents(self.win)
		#self.connectionsClass = connections.Connections(self.win)
		...
		self.rig = rig.Rig()
		#self.worldScene = map.scene.Scene(self)
		self.rigPage_update()
	...
	def close(self):
		...
		if self.win != None:
			...
			self.win.close()
			#self.win.deleteLater() # restore in final release
			self.win = None
	...
	def rigTemplatesMenuUpdate(self):
		...
		def getTemplateFiles():
			templfilesList = os.listdir(rootPath + '/templates/rigs')
			templFiles = []
		...
		for row in range(3):
			for column in range(9):	
				if i < len(standartButtons) :
					b = QtWidgets.QPushButton()
					icon = QtGui.QPixmap(rootPath+'/icons/control_'+standartButtons[i]+".png")
	...
	def initUi(self):
		...
		def setMenu():
			...
			#def currentMenuItemActivate(t=0):
				#self.currentMenuItem = t.text()			
				#if "&" in self.currentMenuItem:
					#return
				#a = self.menuActions[t.text()]
				
				#if self.helpEvent:
					#if self.helpEvent.helpMode:
						#print 222, t.toolTip() #t.text()#, QtGui.QCursor().pos()
				#print 333, cmds.helpLine(self.helpLine, q=1, statusBarMessage=1)
			...
			for sectionListName in sorted(sections):
				...
				for m in sections[sectionListName]:
					...
					#self.menuEvents[utils.formatName(m)] = MenuEventClass()
					#m_action.installEventFilter(self.menuEvents[utils.formatName(m)])						
	
					#dup_m_action = QtWidgets.QAction(self.win)
					#dup_m_action.setText(utils.formatName(m))
					#dup_m_action.triggered.connect(partial(self.action_duplicate, m))
					#dup_m_action.setEnabled(utils.moduleIsHaveScene(m))
					...
					#edit_m_action = QtWidgets.QAction(self.win)
					#edit_m_action.setText(utils.formatName(m))
					#edit_m_action.triggered.connect(partial(self.action_module_edit, m))
					#edit_m_action.setEnabled(utils.moduleIsHaveScene(m))
					
				#self.menuEvents[sectionListName] = MenuEventClass()
				#sub_menu.installEventFilter(self.menuEvents[sectionListName])						
	
			#menu.setStyleSheet("{ font-size:28px; width: 250px; color:white; left: 20px;}")
			#menu.setStyleSheet("QMenu::item {font-family: Arial; font-size: 13pt;}")
			font = menu.font()
			font.setPixelSize(12)
			menu.setFont(font)	
	
			self.win.addModule_btn.setMenu(menu)
			#self.win.addModule_btn.setPopupMode(QtWidgets.QToolButton.InstantPopup)		
		...
		def setToolbar():
			self.win.actionPosers.setIcon(QtGui.QIcon(rootPath+'/ui/icons/posers.svg'))
			self.win.actionControls.setIcon(QtGui.QIcon(rootPath+'/ui/icons/controls.png'))
			self.win.actionJoints.setIcon(QtGui.QIcon(rootPath+'/ui/icons/joints.png'))
			self.win.actionGeometry.setIcon(QtGui.QIcon(rootPath+'/ui/icons/geometry.png'))
			self.win.actionJoints_Template.setIcon(QtGui.QIcon(rootPath+'/ui/icons/joints_template.png'))
			self.win.actionGeometry_Template.setIcon(QtGui.QIcon(rootPath+'/ui/icons/geometry_template.png'))
			self.win.actionGeometry_Reference.setIcon(QtGui.QIcon(rootPath+'/ui/icons/geometry_reference.png'))
			self.win.actionAdvanced.setIcon(QtGui.QIcon(rootPath+'/ui/icons/map.png'))
			self.win.actionPosers_Hierarhy.setIcon(QtGui.QIcon(rootPath+'/ui/icons/posers_hier_on.png'))
			self.win.actionSkeleton_LRA.setIcon(QtGui.QIcon(rootPath+'/ui/icons/axises2.png'))
			self.win.actionPosers_Axises.setIcon(QtGui.QIcon(rootPath+'/ui/icons/axises.png'))
			self.win.actionHelp_Mode.setIcon(QtGui.QIcon(rootPath+'/ui/images/helpBig_icon.png'))
			self.win.actionMove_Tool.setIcon(QtGui.QIcon(rootPath+'/ui/icons/moveTool.png'))
		...
		def setGroupLabels():
			...
			GroupLabel(self.win.inputConnections_label, self.win.inputConnections_groupFrame, self.win.verticalLayout_12)
			
			# Tools group label
			GroupLabel(self.win.skinTools_label, self.win.skinTools_groupFrame, self.win.verticalLayout_66)
			GroupLabel(self.win.curveTools_label, self.win.curveTools_groupFrame, self.win.verticalLayout_72)
			g = GroupLabel(self.win.parentSwitcher_label, self.win.parentSwitcher_groupFrame, self.win.verticalLayout_58)
			...
			g = GroupLabel(self.win.twistConnections_label, self.win.twistConnections_groupFrame, self.win.verticalLayout_10, self.win)		
			g.off = False
			g.mousePressEvent(1)			
		...
		self.win.center_tabWidget.setTabIcon(0, QtGui.QIcon(rootPath+'/ui/icons/dummy_32.png'))
		...
		#self.win.splitter.setSizes([200, 300, 400])
		self.win.splitter.setStretchFactor(0, 0)
		self.win.splitter.setStretchFactor(1, 1)
		#self.win.splitter.setStretchFactor(2, 1)		
		self.win.temp_btn.setVisible(False)	
		#self.win.poseCurrentModule_frame.setVisible(False)	
		self.win.curveTools_frame.setVisible(False)	
		
		#self.win.infoCurrentModule_frame.setVisible(False)	
		#self.win.frame_5.setVisible(False)	
		#self.win.frame_3.setVisible(False)	
		#self.win.options_frame.setVisible(False)	
		#self.win.frame_7.setVisible(False)	
		#self.win.center_frame.setVisible(False)	
		#self.win.delete_btn.setVisible(False)	

		self.win.center_tabWidget.setCurrentIndex(0)
		#self.win.right_tabWidget.setCurrentIndex(0)
		...
		self.win.actionDebug.setChecked(configData['debug'])
		self.win.actionAdvanced.setChecked(configData['advanced'])
		self.win.actionPosers_Hierarhy.setChecked(configData['posersHierarhy'])
		self.toggleMaps(first=True)
		...
		#self.win.controls_tableWidget.setHorizontalHeaderLabels(["Enable", "Name", "Color"])	
		...
		try:
			self.win.controls_tableWidget.horizontalHeader().setResizeMode(1, QtWidgets.QHeaderView.Stretch)
		except:
			self.win.controls_tableWidget.horizontalHeader().setSectionResizeMode(1, QtWidgets.QHeaderView.Stretch)		

		self.win.createCtrlCircle_btn.setIcon(QtGui.QIcon(rootPath+'/icons/control_circle_icon.png'))
		self.win.createCtrlCircle_btn.setIconSize(QtCore.QSize(50, 50))	
		self.win.createCtrlSquare_btn.setIcon(QtGui.QIcon(rootPath+'/icons/control_square_icon.png'))
		self.win.createCtrlSquare_btn.setIconSize(QtCore.QSize(50, 50))	
		self.win.createCtrlBox_btn.setIcon(QtGui.QIcon(rootPath+'/icons/control_box_icon.png'))
		self.win.createCtrlBox_btn.setIconSize(QtCore.QSize(50, 50))	
		self.win.createCtrlSphere_btn.setIcon(QtGui.QIcon(rootPath+'/icons/control_sphere_icon.png'))
		self.win.createCtrlSphere_btn.setIconSize(QtCore.QSize(50, 50))	
		self.win.deleteAddControl_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/delete.png'))
		self.win.deleteAddControl_btn.setIconSize(QtCore.QSize(20, 20))	
		self.win.copyAsChild_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/copyChild.png'))
		self.win.copyAsChild_btn.setIconSize(QtCore.QSize(20, 20))	
		self.win.copySide_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/copySide.png'))
		self.win.copySide_btn.setIconSize(QtCore.QSize(20, 20))	
		
		self.win.loadRig_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/char.png'))
		self.win.loadRig_btn.setIconSize(QtCore.QSize(32, 32))	
		
		self.win.addModule_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/add_module2.png'))
		self.win.addModule_btn.setIconSize(QtCore.QSize(20, 20))	
		self.win.addTemplate_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/add_template.png'))
		self.win.addTemplate_btn.setIconSize(QtCore.QSize(20, 20))	
		self.win.duplicateModule_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/duplicate_icon.png'))
		self.win.duplicateModule_btn.setIconSize(QtCore.QSize(30, 20))	
		self.win.delete_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/delete.png'))
		self.win.delete_btn.setIconSize(QtCore.QSize(20, 20))	
		
		self.win.moduleTemplate_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/save.png'))
		self.win.moduleTemplate_but.setIconSize(QtCore.QSize(20, 20))	
		self.win.replaceModule_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/reload.png'))
		self.win.replaceModule_but.setIconSize(QtCore.QSize(20, 20))	
		self.win.selectPoser_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/select_poser.png'))
		self.win.selectPoser_but.setIconSize(QtCore.QSize(20, 20))	
		self.win.module_axises_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/axises2.png'))
		self.win.module_axises_but.setIconSize(QtCore.QSize(20, 20))	
		self.win.module_vis_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/eye.png'))
		self.win.module_vis_but.setIconSize(QtCore.QSize(20, 20))	
		self.win.module_solo_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/solo.png'))
		self.win.module_solo_but.setIconSize(QtCore.QSize(20, 20))	
		self.win.module_poser_axises_but.setIcon(QtGui.QIcon(rootPath+'/ui/icons/axises.png'))
		self.win.module_poser_axises_but.setIconSize(QtCore.QSize(20, 20))	
		
		# object spaces
		self.win.parents_add_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_plus.png'))
		self.win.parents_add_btn.setIconSize(QtCore.QSize(24, 20))	
		self.win.parents_remove_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/delete.png'))
		self.win.parents_remove_btn.setIconSize(QtCore.QSize(24, 20))	
		self.win.parents_addTarget_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_plus.png'))
		self.win.parents_addTarget_btn.setIconSize(QtCore.QSize(30, 20))	
		self.win.parents_removeTarget_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_minus.png'))
		self.win.parents_removeTarget_btn.setIconSize(QtCore.QSize(30, 20))	
		self.win.parents_targetUp_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_up.png'))
		self.win.parents_targetUp_btn.setIconSize(QtCore.QSize(30, 20))	
		self.win.parents_targetDown_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_down.png'))
		self.win.parents_targetDown_btn.setIconSize(QtCore.QSize(30, 20))	
		
		self.win.parentSwitcher_addTarget_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_plus.png'))
		self.win.parentSwitcher_addTarget_btn.setIconSize(QtCore.QSize(30, 30))	
		self.win.parentSwitcher_removeTarget_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_minus.png'))
		self.win.parentSwitcher_removeTarget_btn.setIconSize(QtCore.QSize(30, 30))	
		self.win.parentSwitcher_targetUp_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_up.png'))
		self.win.parentSwitcher_targetUp_btn.setIconSize(QtCore.QSize(30, 30))	
		self.win.parentSwitcher_targetDown_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/os_target_down.png'))
		self.win.parentSwitcher_targetDown_btn.setIconSize(QtCore.QSize(30, 30))	
		
		# Twists
		self.win.addTwist_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/twist_add.png'))
		self.win.addTwist_btn.setIconSize(QtCore.QSize(30, 20))			
		self.win.addTwistSimple_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/twistSimple_add.png'))
		self.win.addTwistSimple_btn.setIconSize(QtCore.QSize(30, 20))			
		self.win.removeTwist_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/delete.png'))
		self.win.removeTwist_btn.setIconSize(QtCore.QSize(24, 20))			
		
		# Inbetweens
		self.win.inbetweenJoints_image_btn.setIcon(QtGui.QIcon(rootPath+'/ui/images/inbetweenJoints.png'))
		self.win.inbetweenJoints_image_btn.setIconSize(QtCore.QSize(300, 160))			
		#self.win.inbetweenJoints_image_btn.setAutoRaise(False)
		self.win.rotateWeightImage_btn.setIcon(QtGui.QIcon(rootPath+'/ui/images/inbetweenJoints2.png'))
		self.win.rotateWeightImage_btn.setIconSize(QtCore.QSize(300, 154))			
		
		self.win.ib_switchOffsetLocators_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/lamp.png'))
		self.win.ib_switchOffsetLocators_btn.setIconSize(QtCore.QSize(24, 20))	
		
		self.win.ibtw_add_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/ib_plus.png'))
		self.win.ibtw_add_btn.setIconSize(QtCore.QSize(30, 20))		
		self.win.ibtw_remove_btn.setIcon(QtGui.QIcon(rootPath+'/ui/icons/delete.png'))
		...
		self.win.posersColor_btn.setVisible(0)
		self.win.poserColor_label.setVisible(0)
		...
		#self.win.modules_treeWidget.setColumnHidden(1,1)
		...
		for name in ["Global Options", 'Eyes', 'Lids', 'Nose', 'Cheeks', "Mouth", "Lips"]:
			...
			item.setFont(font)	
			item.setIcon(QtGui.QIcon(rootPath+'/ui/icons/module_item_selected2.png'))
			self.win.sections_listWidget.addItem(item)	
				
	def connectSignals(self):
		# rig page
		self.win.newCharacter_btn.clicked.connect(self.create_rig)
		...
		self.win.actionPicker_Edit_Mod.triggered.connect(partial(self.action_picker, edit=True))
		self.win.actionPicker_Anim_Mod.triggered.connect(partial(self.action_picker, edit=False))
		self.win.action_matchRig_joints.triggered.connect(self.action_matchRig)
		self.win.action_matchRig_animation.triggered.connect(partial(self.action_matchRig, True))
		self.win.actionPosers.triggered.connect(self.toggleVis_posers)
		self.win.actionControls.triggered.connect(self.toggleVis_controls)
		self.win.actionJoints.triggered.connect(self.toggleVis_joints)
		self.win.actionGeometry.triggered.connect(self.toggleVis_geo)
		self.win.actionGeometry_Template.triggered.connect(self.toggleTemplate_geo)
		self.win.actionGeometry_Reference.triggered.connect(self.toggleReference_geo)
		self.win.actionJoints_Template.triggered.connect(self.toggleTemplate_joints)
		self.win.actionAdvanced.triggered.connect(self.toggleMaps)
		self.win.actionPosers_Axises.triggered.connect(self.action_posersAxisesToggle)
		self.win.actionSkeleton_LRA.triggered.connect(self.action_jointsAxisesToggle)
		...
		self.win.modules_treeWidget.itemDoubleClicked.connect(self.selectCurModMainPoser)
		...
		self.win.selectPoser_but.clicked.connect(self.selectCurModMainPoser)
		...
		self.win.pointOnCurveLocators_btn.clicked.connect(tools.pointOnCurveLocators)
	...
	def toggleVis_posers(self):
		debugStart(traceback.extract_stack()[-1][2])
		
		state = self.win.actionPosers.isChecked()
		
		for m_name in self.rig.moduleNames:
			if state == 0: # or self.rig.modules[m_name].opposite:
				cmds.hide(m_name+'_posers')
			else:
				cmds.showHidden(m_name+'_posers')
			
			# connect ankle rotate x of the quadrupped module
			#m = utils.getModuleInstance(m_name)					
			#if m.type == 'limbQuadrupped':
				#try:
					#m.connect_ankle_to_posers(state)			
				#except: pass

		debugEnd(traceback.extract_stack()[-1][2])	

	def toggleVis_controls(self):
		debugStart(traceback.extract_stack()[-1][2])

		state = self.win.actionControls.isChecked()
		
		for m in self.rig.moduleNames:
			if state == 0:
	...
	def toggleVis_joints(self):
		debugStart(traceback.extract_stack()[-1][2])

		state = self.win.actionJoints.isChecked()
	...
	def toggleVis_geo(self):
		debugStart(traceback.extract_stack()[-1][2])

		state = self.win.actionGeometry.isChecked()
	...
	def toggleTemplate_joints(self):
		debugStart(traceback.extract_stack()[-1][2])
		
		state = self.win.actionJoints_Template.isChecked()
	...
	def toggleTemplate_geo(self):
		debugStart(traceback.extract_stack()[-1][2])

		state = self.win.actionGeometry_Template.isChecked()
		...
		if state == 0:
			...
			self.win.actionGeometry_Reference.setChecked(False)

		debugEnd(traceback.extract_stack()[-1][2])	

	def toggleReference_geo(self):
		debugStart(traceback.extract_stack()[-1][2])

		state = self.win.actionGeometry_Reference.isChecked()
		...
		if state == 0:
			...
			self.win.actionGeometry_Template.setChecked(False)

		debugEnd(traceback.extract_stack()[-1][2])	

	def toggleMaps(self, first=False):
		return
		debugStart(traceback.extract_stack()[-1][2])
		global configData

		advanced = self.win.actionAdvanced.isChecked()
		
		y = self.win.size().height()
		x = self.win.size().width()
		
		self.win.right_frame.setVisible(advanced)
		
		# get size delta 
		newX = 0
		delta = self.win.right_frame.size().width() 
		if advanced:
			newX = x+546
		else:
			newX = x-delta

		# save advantage toggle data to file	
		configData['advanced'] = advanced
		json_string = json.dumps(configData, indent=4)
		with open(rootPath+'/config.json', 'w') as f:
			f.write(json_string)	
				
		# set size
		self.win.setFixedSize(newX,y)
		
		# set size for first launch
		if first:
			if advanced:
				self.win.setFixedSize(1200,y)
			else:
				self.win.setFixedSize(500,y)
		
		# retore size adjustments			
		self.win.setMinimumSize(0, 0)
		self.win.setMaximumSize(10000, 10000)
		#self.win.layout().setSizeConstraint(QtWidgets.QLayout.SetNoConstraint)
		#self.win.setFixedSize(500,y)
		#self.win.setMinimumSize(0, 0)
		#self.win.setMaximumSize(10000, 10000)		
		self.win.layout().setSizeConstraint(QtWidgets.QLayout.SetDefaultConstraint)
		
		# set window position
		#x = self.screen_resolution.width()
		#y = self.screen_resolution.height()
		#rect = QtCore.QRect(x/3, y/3, 100, 100)
	...
	def	moduleTemplatesMenuUpdate(self):
		...
		def getTemplateFiles():
			templfilesList = os.listdir(rootPath + '/templates/modules')
			templFiles = []
	...
	def compoundModuleMenuUpdate(self):
		...
		m_path = rootPath + '/templates/compoundModules'
		...
		for cData in mData['additionalControlsData']:
			if not cData['mirrored']:
				...
				for attr in attrData:
					...
					except: pass #print p+'.'+attr, "is locked"
	...
	def template_actions(self, action, tName="", forceData=None):
		...
		if action == 'save':
			name, ok = QtWidgets.QInputDialog().getText(self.win, 'Save Module Template', 'Enter template name:', QtWidgets.QLineEdit.Normal, self.curModuleName)
			...
			fullPath = rootPath + '/templates/modules/' + self.curModule.type + '_' + name + ".tmpl"
			...
			try:
				cmds.select(sel)
			except: cmds.select(clear=1)			

		elif action == 'load':
			# read data
			with open(rootPath+'/templates/modules/'+self.curModule.type + '_' + tName+'.tmpl', mode='r') as f:
				mData = json.load(f)
				...
				if self.curModule.symmetrical:
					...
					oldCurItem = self.win.modules_treeWidget.findItems(cur_m.name,QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
			...
			t_name, ok = QtWidgets.QInputDialog().getText(self.win, 'Save Rig Template', 'Enter template name:', QtWidgets.QLineEdit.Normal, self.rig.name)
			...
			fullPath = rootPath + '/templates/rigs/' + t_name + ".tmpl"
			print (fullPath)
			# format data 
			...
			with open(rootPath+'/templates/rigs/'+tName+'.tmpl', mode='r') as f:
				...
				# create modules
				if print_main_messages: print (" -------------------------------- LOAD MODULES ------------------------------------------------ ")
				modulesData = []
				mirroredModulesData = []
				for mData in data['modulesData']:
					if mData['opposite']:
						...
						m = self.addModule(mData['type'], name=mData['name'], options=mData['optionsData'], updateUI=False)
				...
				if print_main_messages: print (" -------------------------------- SET MODULES ------------------------------------------------ ")
				# load modules data
				...
				if print_main_messages: print (" -------------------------------- SET ADD CONTROLS DATA ------------------------------------------------ ")
				# load modules data
				...
				if print_main_messages: print (" -------------------------------- CONNECT MODULES ------------------------------------------------ ")
				# load modules data
				cmds.window(window, e=1, t='Load modules data')
				cmds.progressBar(progressControl, e=1, maxValue=len(modulesData), progress=0)		
				for mData in modulesData:
				...
				if print_main_messages: print (" -------------------------------- MIRROR MODULES ------------------------------------------------ ")
				...
				if print_main_messages: print (" -------------------------------- SET Parents DATA ------------------------------------------------ ")
				# load oss data
				...
				if print_main_messages: print (" -------------------------------- Snap To Parents End Joint DATA ------------------------------------------------ ")
				# load oss data
				...
				if print_main_messages: print (" -------------------------------- SET ATTRIBUTES ------------------------------------------------ ")
				# load modules data
				cmds.window(window, e=1, t='Load modules data')
				cmds.progressBar(progressControl, e=1, maxValue=len(modulesData), progress=0)		
				for mData in modulesData:
				...
				if print_main_messages: print (" -------------------------------- END ------------------------------------------------ ")
			...
			t_name = QtWidgets.QFileDialog.getSaveFileName(self.win, "Save compound module", rootPath+'/templates/compoundModules', "*.ctmpl")[0]
			...
			with open(tName, mode='r') as f:
				...
				if print_main_messages: print (" -------------------------------- Last SET Data MODULES ------------------------------------------------ ")
				# load modules data
				cmds.window(window, e=1, t='Load modules data')
				cmds.progressBar(progressControl, e=1, maxValue=len(modulesData), progress=0)		
				for mData in modulesData:
			...
			path = rootPath+'/templates/modules/'+self.curModule.type + '_' + tName+'.tmpl'
			os.remove(path)
			self.moduleTemplatesMenuUpdate()
			...
			path = rootPath+'/templates/rigs/' + tName+'.tmpl'
			os.remove(path)
			self.rigTemplatesMenuUpdate()
			...
			with open(rootPath + '/controlShapes.cmds', 'w') as f:
				f.write(json_string)					
		...
		for m_name in self.rig.moduleNames:
			for p in cmds.listRelatives(m_name+'_posers', type='transform', allDescendents=1) or []:
				if p.split('_')[-1] == 'poser':
					posers.append(p)
		for p in posers:
			cmds.setAttr(p+'.overrideColor', id)
	...
	def moveTool(self):
		...
		def catch():
			...
			#imagemap = QtGui.QPixmap(rootPath+'/modules/%s/screen.png' %self.curModule.type)
		...
		self.mWin.removeEventFilter(self.rigBrushEvent)
		#print "moveTool 0"
		cmds.draggerContext(ctx, pressCommand=catch, dragCommand=moving, releaseCommand=drop, name=ctx, cursor='crossHair')
		cmds.setToolTo(ctx)
		#print "moveTool 1"
	...
	def saveControlShape(self):
		...
		name, ok = QtWidgets.QInputDialog().getText(self.win, 'Save Control Shape', 'Enter shape name:', QtWidgets.QLineEdit.Normal, sel[0])
	...
	def create_rig(self, name=""):
		debugStart(traceback.extract_stack()[-1][2])

		if utils.groupExistsWarning('main'):
			cmds.warning('Group main is exists')
			return
		if utils.groupExistsWarning('rig'):
			cmds.warning('Group rig is exists')
			return
		if utils.groupExistsWarning('geo'):
			cmds.warning('Group geo is exists')
			return

		self.rig.create()
		
		#self.worldScene.create()
	...
	def rigPage_update(self):
		...
		if charExist:
			name = self.rig.name
			#self.win.characterName_lineEdit.setText(name)
			self.updateModulesTree()
			...
			self.win.actionPosers_Axises.setChecked(cmds.getAttr(self.rig.root_group+'.posersAxises'))
			self.win.actionSkeleton_LRA.setChecked(cmds.getAttr(self.rig.root_group+'.jointsAxises'))
		else:
			self.win.tabWidget.setCurrentIndex(0)
			#self.win.characterName_lineEdit.setText("")
		...
		#self.win.nodeOptions_tab.setEnabled(charExist)
		#self.win.modulesMap_tab.setEnabled(charExist)
		#self.win.sets_tab.setEnabled(charExist)
		self.win.left_frame.setEnabled(charExist)
		
		#self.win.toolBar.setEnabled(charExist)
		...
		#self.win.actionHelp_Mode.setEnabled(charExist)	
		...
		if charExist:
			# init toolbar
			if len(self.rig.moduleNames) > 0:
				self.win.actionPosers.setChecked(cmds.getAttr(self.rig.moduleNames[0]+'_posers.v'))
				self.win.actionControls.setChecked(cmds.getAttr(self.rig.moduleNames[0]+'_controls.v'))
	
			self.win.actionJoints.setChecked(cmds.getAttr('skeleton.v'))
			self.win.actionGeometry.setChecked(cmds.getAttr('geo.v'))
			self.win.actionJoints_Template.setChecked(cmds.getAttr('skeleton.template'))
			self.win.actionGeometry_Template.setChecked(cmds.getAttr("geo.overrideEnabled") and cmds.getAttr("geo.overrideDisplayType") == 1)
			self.win.actionGeometry_Reference.setChecked(cmds.getAttr("geo.overrideEnabled") and cmds.getAttr("geo.overrideDisplayType") == 2)		

			modules_list = cmds.listRelatives("modules") or []
			published = modules_list != [] and self.rig.moduleNames == []
			self.win.modules_tab.setEnabled(not published)
			self.win.rig_tab.setEnabled(not published)
			self.win.addControls_tab.setEnabled(not published)
			self.win.twists_tab.setEnabled(not published)
			self.win.inbetweens_tab.setEnabled(not published)
			self.win.os_tab.setEnabled(not published)
	...
	def rename_rig(self):
		...
		newName, ok = QtWidgets.QInputDialog().getText(self.win, 'Rename Rig', 'Enter rig name:', QtWidgets.QLineEdit.Normal, oldName)
	...
	def updateModulesTree(self):
		...
		# save current module name for reselect it
		tempName =  self.curModuleName

		# clear old data
		self.win.modules_treeWidget.clear()
		self.win.modules_treeWidget.setColumnCount(2)
		#print self.win.modules_treeWidget.currentColumn()

		if self.rig.moduleNames == []: 
			debugEnd(traceback.extract_stack()[-1][2])	
			return
		...
		for name in self.rig.moduleNames:
			...
			if not m.parent:
				...
				if cmds.getAttr(m.name+"_mod.v"):
					item.setIcon(1, QtGui.QIcon(rootPath+'/ui/icons/module_item_selected2.png'))
				#item.setText(0,"SSSS")
				#item.setText(1,"FFFF")				
				self.win.modules_treeWidget.addTopLevelItem(item)

			else:
				parents = self.win.modules_treeWidget.findItems(parentModuleName, QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)
				item = QtWidgets.QTreeWidgetItem([name])
				if cmds.getAttr(m.name+"_mod.v"):
					item.setIcon(1, QtGui.QIcon(rootPath+'/ui/icons/module_item_selected2.png'))
				if parents:
					...
					cmds.warning("Missed parent " + name)
				
			
			item.setIcon(0, QtGui.QIcon(rootPath+'/ui/icons/module_%s.png' %m.type))
		...
		try:
			...
			self.win.modules_treeWidget.setCurrentItem(oldCurItem)
		except: self.win.moduleOptions_tab.setEnabled(False)
	...
	def updateModulePage(self, moduleNameOrItem, prevItem="", updateMap=False):
		...
		#self.win.addControls_frame.setEnabled(1)
		...
		#self.win.parentPoser_lineEdit.setText(utils.getModuleNameFromAttr(m.parent))
		...
		#if not self.curModule.isConnected():
			#self.win.makeSymmetry_btn.setEnabled(False)
		#else:
		...
		if len(dup) > 0:
			cmds.warning("Duplicates in controls: %s" %dup)
			return

		for i in range (len(controls)):
			#item = QtWidgets.QTableWidgetItem('control_'+str(i+1))
			...
			#self.win.controls_tableWidget.setItem(count, 0, item)

			# add internal name
			int_name = cmds.getAttr(control+".internalName")
			...
			shapes = cmds.listRelatives(control, s=1)
			...
			# add color
			color = cmds.getAttr(utils.getShape(control)+'.overrideColor')
			item = QtWidgets.QTableWidgetItem() #str(color))
			self.win.controls_tableWidget.setItem(count, 2, item)	
			#item.setFlags(not QtCore.Qt.ItemIsEditable)
			item.setFlags(QtCore.Qt.ItemIsSelectable)

			shape = cmds.listRelatives(control, s=1)[0]
			colorId = cmds.getAttr(shape+'.overrideColor')
			#print control, shape, colorId
		...
		for x in range(self.win.options_lay.count()):
			w = self.win.options_lay.itemAt(x).widget()
			w.deleteLater()		
		path = rootPath+'//modules//'+m.type+'//'+m.type+'_widget.ui'
	...
	def addModule(self, moduleType, name="", options={}, updateUI=True, nodePosition=""):
		...
		if name == "":
			name, ok = QtWidgets.QInputDialog.getText(self.win, "Add Module", "Please enter new module name", QtWidgets.QLineEdit.Normal, moduleType)
		...
		if sys.version[0] == "2":
			exec('import modules.%s.%s' %(moduleType,moduleType)) # import modules.moduleA.moduleA
			exec('reload(modules.%s.%s)' %(moduleType,moduleType)) # reload(modules.moduleA.moduleA)
			m = eval('modules.%s.%s.%s(name)' %(moduleType, moduleType, moduleTypeCap))	# m = modules.moduleA.moduleA.ModuleA(name)
		else:
			exec('import rigStudio2.modules.%s.%s' %(moduleType,moduleType)) # import modules.moduleA.moduleA
			exec('importlib.reload(rigStudio2.modules.%s.%s)' %(moduleType,moduleType)) # reload(modules.moduleA.moduleA)
			m = eval('rigStudio2.modules.%s.%s.%s(name)' %(moduleType, moduleType, moduleTypeCap))	# m = modules.moduleA.moduleA.ModuleA(name)
		m.create(options)
		
		self.rig.modules[name] = m
		self.rig.updateModuleNames()

		cmds.parent(m.root, 'modules')
		...
		# add map node
		#self.worldScene.addNode(moduleType, name, position=nodePosition)
		#self.curModule.node = self.worldScene.nodes[self.curModuleName]
		
		if updateUI:
			self.updateModulesTree()	
			self.selectCurModMainPoser()
			self.updatePosersColor(color_id)
		#self.action_posersHierarhy()
	...
	def duplicateModule(self, module=""):
		...
		self.setAddControlsData(data, self.curModuleName)
		self.addControls_updateTree()
	...
	def deleteModule(self, moduleName="", updateUI=True):
		...
		self.rig.moduleNames.remove(moduleName)
		...
		if len(self.rig.moduleNames) > 0:
			self.curModuleName = self.rig.moduleNames[-1]
			self.curModule = self.rig.modules[self.curModuleName]
		else: 
			self.curModuleName = None
			self.curModule = None
	...
	def selectCurModMainPoser(self):
		...
		cmds.select(self.curModuleName+"_mainPoser")		
	...
	def makeSymmetryModule(self, moduleName="", updateUI=True, test=False):
		...
		if updateUI:
			...
			try:
				newCurItem = self.win.modules_treeWidget.findItems(utils.getOppositeObject(newModuleName),QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
	...
	def renameModule(self, newName="", moduleName=""):
		...
		# get new name
		if newName == "":
			newName, ok = QtWidgets.QInputDialog().getText(self.win, 'Rename Module', 'Enter module name:', QtWidgets.QLineEdit.Normal, self.curModuleName)
		...
		if moduleName == "":
			...
			try:
				newCurItem = self.win.modules_treeWidget.findItems(moduleName,QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
				self.win.modules_treeWidget.setCurrentItem(newCurItem)			
			except: pass
		...
		if sym:
			...
			try:
				newCurItem = self.win.modules_treeWidget.findItems(utils.getOppositeObject(self.curModuleName),QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
				self.win.modules_treeWidget.setCurrentItem(newCurItem)				
			except: pass
	...
	def rebuildModule(self, options={}, moduleType=""):
		...
		for m_name in self.rig.modules:
			m = self.rig.modules[m_name]
			if self.curModuleName == utils.getModuleNameFromAttr(m.parent):
				children[m_name] = [m.parent, m, m.isSnapped()]
				
		# save data
		curModuleData = self.curModule.getData()
		m_name = self.curModuleName
				...
				if mData:
					for d in mData:
						...
						if cmds.objExists(obj):
							if utils.getModuleNameFromAttr(obj) == self.curModuleName:
		...
		oldCurModuleName = self.curModuleName
		...
		self.addModule(curModuleData['type'], m_name, options, updateUI=False)#, nodePosition=curModuleData['nodePosition'])
		...
		for twData in curModuleData['twistsData']:
			twist.Twist(self.win).twists_add(twData, self.curModuleName)

		# set to old seleted mudule and update ui
		self.curModuleName = oldCurModuleName
		self.curModule = self.rig.modules[self.curModuleName]
		self.updateModulesTree()
		self.addControls_updateTree()
	...
	def additionalControl_add(self, shape, name=""):
		...
		if name == "":
			name, ok = QtWidgets.QInputDialog().getText(self.win, 'Create Additional Control', 'Enter control name:', QtWidgets.QLineEdit.Normal, 'ctrl')
		...
		for m_name in self.rig.modules:
			...
			for c in m.getAdditionalControls():
				...
				if c.parent == "":
					...
					parents = self.win.additionalControls_treeWidget.findItems(c.parent, QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0) or []
					if len(parents) > 0:
	...
	def addControls_renameControl(self, newName="", moduleName=""):
		...
		if newName == "":
			newName, ok = QtWidgets.QInputDialog().getText(self.win, 'Rename Add Control', 'Enter control name:', QtWidgets.QLineEdit.Normal, self.curAddControl.name)
		...
		# select in tree
		newCurItem = self.win.additionalControls_treeWidget.findItems(newName,QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
	...
	def addControls_mirrorControl(self, curAddControl=None):
		...
		if mirrored_module:
			...
			utils.connectByMatrix(mirroredControl.name+'_addPoser', [comp2, curAddControl.name+'_addPoser', comp], ['outputMatrix', 'matrix', 'outputMatrix'], m.name)			
		...
		try:
			newCurItem = self.win.additionalControls_treeWidget.findItems(utils.getOppositeObject(mirroredControl.name),QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
			self.win.additionalControls_treeWidget.setCurrentItem(newCurItem)			
		except: pass
	...
	def addControls_setParent(self):
		...
		# select table item
		newCurItem = self.win.additionalControls_treeWidget.findItems(self.curAddControlName,QtCore.Qt.MatchExactly | QtCore.Qt.MatchRecursive, 0)[0]
		self.win.additionalControls_treeWidget.setCurrentItem(newCurItem)			
	...
	def action_module_edit(self, name):
		...
		mel.eval('NewScene;')
		m_file = rootPath + '//modules//' + name + '//' + name + '.ma'	
		cmds.file(m_file, o=1)
	...
	def action_cacheManager(self):
		import rigCache
		reload(rigCache)
		import rigCache.main
		reload(rigCache.main)

		global rigCache_win
		try:
			rigCache_win.close()
			rigCache_win.deleteLater()
		except: pass

		rigCache_win = rigCache.main.RigCacheWindow()
		rigCache_win.show()
	...
	def action_about(self):
		...
		# logo
		imagemap = QtGui.QPixmap(rootPath+'/icons/rs_logo_about.png')
	...
	def action_moduleBuilder(self):
		...
		if sys.version[0] == "2":
			...
			import importlib
			import rigStudio2.moduleBuilder as moduleBuilder
	...
	def action_picker(self, edit):
		debugStart(traceback.extract_stack()[-1][2])
		
		if sys.version[0] == "2":
			import rigStudio2.picker as picker
			reload(picker)
		else:
			import importlib
			import rigStudio2.picker as picker
		...
		picker.main.run(edit=edit)

	def action_matchRig(self, anim=False):
		debugStart(traceback.extract_stack()[-1][2])
		...
		#rigStudio2.picker.main.run(edit=False, picker_name="mr", match_rig=True, match_scene=anim)
		picker.main.run(edit=False, rigFromJoints=True)
	...
	#def action_posersHierarhy(self, state="none"):
		#debugStart(traceback.extract_stack()[-1][2])
		
		#if state == "none":
			#state = self.win.actionPosers_Hierarhy.isChecked()
		#configData['posersHierarhy'] = state
		...
		#all_posers = []
		#for m_name in self.rig.moduleNames:
			#posers = self.rig.modules[m_name].getPosers()
			#for p in posers:
				## save position
				#initMatrix = cmds.xform(p, query=True, ws=True, m=True)
				#if p.split('_')[0] != "r":
					#all_posers.append([p, initMatrix])
		
		#for p_data in all_posers:
			##print p_data[0]
			#cmds.setAttr(p_data[0]+".inheritsTransform", state)
			
		#for p in all_posers:
			##print p[0]
			## restore position
			#cmds.xform(p[0], ws=True, m=p[1])
	...
	def action_jointsAxisesToggle(self):
		state = self.win.actionSkeleton_LRA.isChecked()
	
		utils.setUserAttr("main", "jointsAxises", state, type="bool", lock=True, keyable=False, cb=False)
	...
	def actionDriven_Groups(self):
		...
		if sys.version[0] == "2":
			import rigStudio2.driver as driver
			reload(driver)
		else:
			import importlib
			import rigStudio2.driver as driver
...
def run(show=True):
	...
	# update map labels positions
	cmds.refresh()
	try:
		rs_win.map.curScene.view.updateLabelsPosition()
	except: pass
