...
#
# License: Core code distributed under the MIT License and available for free use.
# Additional modules and expanded functionality may be offered either for free or sold separately.
#
# Dependencies:
# - Autodesk Maya version 2022-2025
...
class MainWindow:
    def __init__(self):
        ...
        # self.win.splitter.setSizes([10, 400])
    ...
    def initUi(self):
        ...
        def setUI():
            ...
            self.win.addControls_splitter.setStretchFactor(0, 0)
            self.win.addControls_splitter.setStretchFactor(1, 1)
            self.win.twists_splitter.setStretchFactor(0, 0)
            self.win.twists_splitter.setStretchFactor(1, 1)
            self.win.inbetweens_splitter.setStretchFactor(0, 0)
            self.win.inbetweens_splitter.setStretchFactor(1, 1)
            self.win.os_splitter.setStretchFactor(0, 0)
            self.win.os_splitter.setStretchFactor(1, 1)
        ...
        self.win.actionLicense.setVisible(False)
        self.win.menuRig.setVisible(False)
        
        # if self.lic_status == 1:
        #     self.win.setWindowTitle("Rig Studio"+ utils.get_version().split("Version")[-1])
        # elif self.lic_status == 7 :
        #     self.win.setWindowTitle("Rig Studio"+ utils.get_version().split("Version")[-1][:-1]+" (СМФ)")
        # else:
        #     self.win.setWindowTitle("Rig Studio 3 (Free version) "+ utils.get_version())
        self.win.setWindowTitle("Rig Studio 3 "+ utils.get_version())
    ...
    def rigTemplatesMenuUpdate(self):
        ...
        if len(templateNames) > 0:
            ...
            for t in templateNames:
                ...
                m_action.triggered.connect(partial(self.templateClass.rig_delete, t))
    ...
    def controlShapesUpdate(self):
        ...
        if len(self.controlShapes_data) > 0:
            ...
            for name in self.controlShapes_data:
                ...
                m_action.triggered.connect(partial(self.shape_delete, name))
                delete_menu.addAction(m_action)
    ...
    def moduleTemplatesMenuUpdate(self):
        ...
        loadRigTepl_action.setText("Load Template..")
        menu.addAction(loadRigTepl_action)
        loadRigTepl_action.triggered.connect(partial(self.templateClass.module_load, custom=True))
    ...
    def compoundModuleMenuUpdate(self):
        ...
        for t in templates:
            ...
            for f in templates[t]:
                ...
                m_action.triggered.connect(partial(self.templateClass.compound_delete, f))
                sub_menu.addAction(m_action)
    ...
    def shape_delete(self, name):
        del self.controlShapes_data[name]
        ...
        print("Control shape %s was deleted" % name)
    ...
    def homePage(self):
        import webbrowser
        # webbrowser.open('http://rigstudio.ru', new=0, autoraise=True)
        webbrowser.open('https://boosty.to/pavelcrow', new=0, autoraise=True)

    def tutorialsPage(self):
        import webbrowser
        webbrowser.open('https://www.youtube.com/watch?v=1sMBoEvhdB8&list=PLpI9_c3KaSFSpNmpTsIbiHxBN1sDSfweD', new=0, autoraise=True)
    ...
    def rigPage_update(self):
        ...
        if charExist:
            ...
            self.win.actionSkeleton_LRA.setChecked(int(self.rig.jointsAxises))
        else:
            self.win.tabWidget.setCurrentIndex(0)

        self.win.jointsDisplayScale_lineEdit.setText(str(round(cmds.jointDisplayScale(q=1), 3)))
        self.win.jointsDisplayScale_slider.setValue(cmds.jointDisplayScale(q=1)*100)

        # enable/disable ui widgets
        self.win.rigPage_frame.setEnabled(charExist)
        self.win.Skeleton_frame.setEnabled(not charExist)
        ...
        if self.rig.singleHierarhy:
            self.win.singleHierarhy_radioButton.setChecked(True)
        else:
            self.win.moduleJoints_radioButton.setChecked(True)
        ...
        if charExist:
            ...
            if len(self.rig.modules) > 0:
                self.win.actionPosers.setChecked(int(self.rig.posersVis))
                self.win.actionControls.setChecked(int(self.rig.controlsVis))
    ...
    def create_rig(self):
        state = self.win.singleHierarhy_radioButton.isChecked()
        
        self.rig.create(state)

        # utils.setUserAttr(self.rig.root, 'singleHierarhy', state, type="bool", lock=1)
        
        # cmds.setAttr(self.rig.root+'.singleHierarhy', state)
        # print(66, cmds.getAttr(self.rig.root+'.singleHierarhy'))
    ...
    def posersSize_resetUi(self):
        ...
        v = float(self.win.posersSize_lineEdit.text())
        cmds.setAttr(self.rig.root + '.posersSize', 1)
    ...
    def jointsSize_update(self, value=0, update=True, manual_v=0):
        ...
        if manual_v:
            ...
            self.win.jointsSize_slider.setValue(v*100)
        else:
    ...
    def jointsSize_resetUi(self):
        # print("reset ui")
        v = float(self.win.jointsSize_lineEdit.text())
        cmds.setAttr(self.rig.root + '.jointsSize', 1)
    ...
    def jointsDisplayScale_update(self, value=0, update=True, manual=0):
        # print("update", value, update, manual_v)
        if manual:
            v = float(self.win.jointsDisplayScale_lineEdit.text())
            self.win.jointsDisplayScale_lineEdit.setText(str(round(v, 3)))
            cmds.jointDisplayScale(v)
            self.win.jointsDisplayScale_slider.setValue(v*100)
        else:
            v = self.win.jointsDisplayScale_slider.value() * 0.01
            cmds.jointDisplayScale(v)
            self.win.jointsDisplayScale_lineEdit.setText(str(round(v, 3)))
    ...
    def updateModulesTree(self): #
        ...
        if cur_module_name: 
            self.selectModuleInList(cur_module_name)
    ...
    def duplicateModule(self, module=""): #
        ...
        data = self.templateClass.get_cur_module_chain_data()

        rootModule = self.templateClass.load_modules(data, load='rig')

        self.updateModulesTree()

		# select root module
        self.selectModuleInList(rootModule)

        return self.curModule, data
    ...
    def deleteModule(self, moduleName="", updateUI=True): #
        ...
        # remove ibtws
        ibtws_data = self.ibtwClass.getIbtwsData()
    ...
    def makeSymmetryModule(self, moduleName="", updateUI=True, mirrored_modules=[], root_module=True):
        ...
        print("Make Symmetry", module.name)
        
        oldModule_name = module.name
        oldModule = module
        ...
        if side != "l":
            ...
            l_new_name = utils.incrementModuleNameIfExists(l_new_name)
        ...
        if root_module:
            modulesRename = {oldModule_name:l_new_name}
            
            childern_modules = self.rig.getModuleChildren(module.name)

            # create rename dict
            for m_name in childern_modules:
                # set names
                side = utils.getObjectSide(m_name)
                if side != "l":
                    new_m_name = "l_" + m_name
                    new_m_name = utils.incrementModuleNameIfExists(new_m_name)
                else:
                    new_m_name = m_name

                modulesRename[m_name] = new_m_name
        
            # save twists data
            module_twists = self.twistClass.getTwistsFromModule(module.name)
            twists_data = []
            for tw_name in module_twists:
                tw_data = self.twistClass.getData(tw_name)
                twists_data.append(tw_data)

            # save ibtws data
            childern_modules.append(module.name)
            ibtwsData = self.ibtwClass.getIbtwsData(childern_modules)

            # remove ibtws
            for ibtw_data in ibtwsData:
                self.ibtwClass.remove(ibtw_data["name"])
            
            # remove twists
            if not moduleName:
                for twData in twists_data:
                    self.twistClass.twists_remove(twData["name"])
        
        if not parent and updateUI:
        ...
        for d in data['parents']:
            self.curParents.os_deleteConstraint(d)
        ...
        # module.setData(data, load="options")
        ...
        # add add controls
        addControls = oldModule.getAdditionalControls(rootControlsOnly=True)
        ...
        if children:
            for child in children:
                self.makeSymmetryModule(child, mirrored_modules=mirrored_modules, root_module=False)
        
        if root_module:
            # rename data
            renamed_twData = self.twistClass.renameData(twists_data, modulesRename)
            renamed_ibtwsData = self.ibtwClass.renameData(ibtwsData, modulesRename)

            # add twists
            for twData in renamed_twData:
                self.twistClass.twists_add(twData, module.name)

            # add ibtws
            for ibtw_data in renamed_ibtwsData:
    ...
    def replaceModule(self, moduleType):
        ...
        curModuleData = self.curModule.getData()
        m_name = self.curModule.name
        twistsData = self.twistClass.getTwistsData([self.curModule.name])
        ibtwsData = self.ibtwClass.getIbtwsData([self.curModule.name])
    ...
    def addControls_updateTree(self):
        ...
        for m_name in self.rig.modules:
            ...
            # create parent control top items if not in items and add to table
            top_items = {}
            for c in mod.getAdditionalControls():
                name = c.name
                if c.parent not in items and c.parent not in top_items:
                    parent_item = QtWidgets.QTreeWidgetItem([c.parent])
                    top_items[c.parent] = parent_item
            ...
            # fill items hierarchy
            for c in mod.getAdditionalControls():
                name = c.name
                item = items[name]
                
                if c.parent in items:
                    parent_item = items[c.parent]
                    parent_item.addChild(item)

                else:
                    parent_item = top_items[c.parent]
                    parent_item.addChild(item)
    ...
    def addControls_mirrorControl(self, curAddControl=None): #
        ...
        if mirrored_module:
            ...
            cmds.setAttr(mirroredControl.name + '_addPoser.scaleY', lock=0)
            cmds.setAttr(mirroredControl.name + '_addPoser.scaleZ', lock=0)
    ...
    def action_debug(self):
        self.configData['debug'] = self.win.actionDebug.isChecked()
        utils.setDebug(self.configData['debug'])
    ...
    def action_license(self):
        ...
        def updateStatusText():
            self.licWin.version_label.setText('Core code distributed under the MIT License and available for free use.')
            return
    ...
    def action_moduleBuilder(self):
        # if not self.full:
        # if not self.lic_status == 7 and not self.lic_status == 1:
        #     QtWidgets.QMessageBox.information(self.win, "Sorry", "This feature is available in full version only.")
        #     return

        from rigStudio3 import moduleBuilder
        ...
        self.moduleBuilderWin = load_ui_widget(self.moduleBuilderUiFilePath, parent=self.win.parent())
