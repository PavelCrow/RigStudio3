...
if sys.version[0] == "2":
	import utils, main, twist, additionalControl, parents, inbetweens
else:
	import importlib
	import rigStudio2.utils as utils	
	import rigStudio2.main as main	
	import rigStudio2.twist as twist	
	import rigStudio2.additionalControl as additionalControl	
	import rigStudio2.parents as parents	
	import rigStudio2.inbetweens as inbetweens	
...
#for p in os.environ[ 'MAYA_SCRIPT_PATH' ].split(';'):
	#print p
#print mel_scripts_path
#print mel_scripts_path in os.environ[ 'MAYA_SCRIPT_PATH' ]
#print rootPath.replace('/', '//')
...
class Module(object):
	def __init__(self):
		...
		debugEnd(traceback.extract_stack()[-1][2], self.name)

	def connectSignals(self, mainInstance, w):	
		pass

	def create(self, options={}):
		debugStart(traceback.extract_stack()[-1][2], self.name)
		
		self.root = self.name + "_mod"
		
		# import
		#print ("----- Import module", self.type, "-----", self.name)
		...
		# 	createControlSet(self.name+'_moduleControlSet')
		...
		# posers materials
		if not cmds.objExists(self.name+"_red_rsMat"):
			return
		...
		if cmds.objExists("red_rsMat"):
			...
			for o in cmds.listRelatives(self.name+"_posers", allDescendents=1, type='transform'):
				...
				#elif o.split("_")[-1] in ["poser", "addPoser"]:
					#cmds.sets(o+"NurbsShape", e=1, forceElement='red_rsSG')
					#cmds.select(o)
					#cmds.hyperShade(assign= "red_rsMat")
		else:
			cmds.rename(self.name+"_red_rsMat", "red_rsMat")
			cmds.rename(self.name+"_green_rsMat", "green_rsMat")
			cmds.rename(self.name+"_blue_rsMat", "blue_rsMat")
			cmds.rename(self.name+"_red_rsSG", "red_rsSG")
			cmds.rename(self.name+"_green_rsSG", "green_rsSG")
			cmds.rename(self.name+"_blue_rsSG", "blue_rsSG")			
		
		if cmds.objExists(self.name+"_black_rsMat"):
			if cmds.objExists("black_rsMat"):
				cmds.delete(self.name+"_black_rsMat")
				if cmds.objExists(self.name+"_lines_group"):
				...
				cmds.rename(self.name+"_black_rsMat", "black_rsMat")
				cmds.rename(self.name+"_black_rsSG", "black_rsSG")
		...
		for p in pm.listRelatives(self.name + "_posers", allDescendents=1):
			if p.split("_")[-1] in ['poser', 'mainPoser']:
				try:
					...
					p.size.set(size)
				except: print ("Cannot set size attr in the poser")
	...
	def getParents(self):
		debugStart(traceback.extract_stack()[-1][2], self.name, True)
	...
	def setData(self, data, sym=False, namingForce=False): 
		debugStart(traceback.extract_stack()[-1][2], self.name)
		#print 333333, self.name, namingForce
